...
// TODO remove after v1.7 is released
func (e *Endpoint) BPFConfigMapPath() string {
	return bpf.LocalMapPath("cilium_ep_config_", e.ID)
}
...
// RemoveOldMaps removes any maps which were previously used by Cilium, but
// which have been deprecated / are no longer used.
func (e *Endpoint) RemoveOldMaps() {
	maps := []string{e.BPFConfigMapPath()}
	for _, oldMap := range maps {
		if _, err := os.Stat(oldMap); err == nil {
			if err = os.RemoveAll(oldMap); err == nil {
				log.WithField(logfields.BPFMapPath, oldMap).Info("removed legacy map used by endpoint")
			}
		}
	}
}
